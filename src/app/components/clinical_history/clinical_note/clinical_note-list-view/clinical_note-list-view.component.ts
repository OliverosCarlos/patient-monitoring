...
export class ClinicalNoteListViewComponent implements OnInit {
  ...
  group_by(options:any[]){
    ...
    this.dataSource.data.forEach(opt => {
      if ( grouped_list.find( (item) => {return item.patient_name == opt.patient.full_name}) == undefined) {
        grouped_list.push({
          'patient_name': opt.patient.full_name,
          'children': [opt]
        });
      }else{
        grouped_list.map(item => 
          {
            if(item.patient_name == opt.patient.full_name){
