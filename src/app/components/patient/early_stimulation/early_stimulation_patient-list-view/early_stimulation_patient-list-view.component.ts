...
@Component({
  selector: 'app-early_stimulation_patient-list-view',
  templateUrl: './early_stimulation_patient-list-view.component.html',
  styleUrls: ['./early_stimulation_patient-list-view.component.scss']
})
export class EarlyStimulationPatientListViewComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  getAll(data_search:any){
    ...
    this.backendService.getAll(PATIENT.EARLY_STIMULATION, data_search).subscribe({
  ...
  viewRecord(emotion:any){
    this.router.navigate(['main','patients','early-stimulation','show',emotion.id]);
  }
